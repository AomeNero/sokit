...
:BaseForm(p, f),m_client(0),m_msgListPanel(new MsgListPanel(this))
{
	m_ui.setupUi(this);
}
...
ClientForm::~ClientForm()
{
	...
	if (m_msgListPanel)
	{
		m_msgListPanel->saveMessages();
		delete m_msgListPanel;
	}
}
...
bool ClientForm::initForm()
{
	...
	// Add message list panel to layout
	QHBoxLayout* mainLayout = new QHBoxLayout();
	// Create a widget to hold the original content
	QWidget* contentWidget = new QWidget();
	// Transfer the existing layout to the content widget
	QVBoxLayout* originalLayout = qobject_cast<QVBoxLayout*>(layout());
	if (originalLayout) {
		contentWidget->setLayout(originalLayout);
	}
	// Add the content widget and message list panel to the new main layout
	mainLayout->addWidget(contentWidget, 0); // Left side fixed size
	mainLayout->addWidget(m_msgListPanel, 1); // Right side flexible
	mainLayout->setContentsMargins(0, 0, 0, 0);
	mainLayout->setSpacing(0);
	setLayout(mainLayout);

	// Set send callback
	m_msgListPanel->setSendCallback([this](const QString& data) {
		send(data, "");
	});

	// Connect output signal
	connect(m_msgListPanel, SIGNAL(output(const QString&)), this, SIGNAL(output(const QString&)));

	// Set storage prefix and load messages
	m_msgListPanel->setStoragePrefix("client");
	m_msgListPanel->loadMessages();
